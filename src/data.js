export const data = [
  {
    slug: "almondegas",
    title: "Almôndegas",
    category: "Dia a dia",
    price: "21,99",
    ingredients:
      "Tomate, carne bovina, cebola, extrato de tomate, alho, sal, óleo de soja, pimenta em pó, salsa, tomilho e manjerona.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "497",
    portion: "100",
    calories: { g: "83", vd: "3" },
    carbohydrate: { g: "3,4", vd: "1" },
    sugars: { g: "1,8", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "10", vd: "16" },
    fat: { g: "3,2", vd: "4" },
    saturated: { g: "1,4", vd: "6" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "0,9", vd: "3" },
    sodium: { g: "166", vd: "7" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "bife-a-milanesa",
    title: "Bife à Milanesa",
    category: "Dia a dia",
    price: "25,99",
    ingredients:
      "Carne bovina, ovo, óleo de soja, farinha de rosca, farinha de trigo, sal e pimenta em pó.",
    allergic:
      "Alérgicos: Contém ovo, derivados de trigo e soja. Pode conter leite. Contém glúten.",
    weight: "497",
    portion: "100",
    calories: { g: "392", vd: "26" },
    carbohydrate: { g: "12", vd: "6" },
    sugars: { g: "0", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "32", vd: "82" },
    fat: { g: "24", vd: "48" },
    saturated: { g: "6", vd: "39" },
    trans: { g: "0,4", vd: "30" },
    fiber: { g: "0,7", vd: "4" },
    sodium: { g: "479", vd: "31" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "feijao",
    title: "Feijão",
    category: "Dia a dia",
    price: "14,99",
    ingredients:
      "Água, feijão, cebola, óleo de soja, alho, sal, tomilho, manjerona e louro.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "587",
    portion: "100",
    calories: { g: "185", vd: "23" },
    carbohydrate: { g: "31", vd: "26" },
    sugars: { g: "0", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "11", vd: "56" },
    fat: { g: "2,3", vd: "9" },
    saturated: { g: "0,4", vd: "5" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "11", vd: "112" },
    sodium: { g: "86", vd: "11" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "frango-xadrez",
    title: "Frango Xadrez",
    category: "Dia a dia",
    price: "25,99",
    ingredients:
      "Filé de frango, pimentão verde, cebola, shoyo, amendoim, água, óleo de soja e amido de milho.",
    allergic:
      "Alérgicos: Contém amendoim e derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "177", vd: "9" },
    carbohydrate: { g: "7,4", vd: "2" },
    sugars: { g: "1,9", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "19", vd: "38" },
    fat: { g: "8,5", vd: "13" },
    saturated: { g: "1,5", vd: "8" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "2,6", vd: "10" },
    sodium: { g: "847", vd: "42" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "panqueca-de-frango",
    title: "Panqueca de Frango com Requeijão",
    category: "Dia a dia",
    price: "21,99",
    ingredients:
      "Tomate, carne de frango, requeijão cremoso, farinha de trigo, cebola, óleo de soja, ovo, extrato de tomate, alho, sal, salsa, tomilho, manjerona e noz moscada.",
    allergic:
      "Alérgicos: Contém derivados de trigo e leite, soja e ovo. Contém glúten. Contém lactose.",
    weight: "481",
    portion: "100",
    calories: { g: "188", vd: "9" },
    carbohydrate: { g: "10", vd: "3" },
    sugars: { g: "1,7", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "12", vd: "24" },
    fat: { g: "11", vd: "17" },
    saturated: { g: "3,6", vd: "18" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "1", vd: "4" },
    sodium: { g: "224", vd: "11" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "ragu-de-carne",
    title: "Ragu de Carne de Panela",
    category: "Dia a dia",
    price: "24,99",
    ingredients:
      "Tomate, carne bovina, cebola, extrato de tomate, óleo de soja, alho, sal, pimenta em pó, louro, alecrim, salsa, tomilho e manjerona.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten. Contém lactose.",
    weight: "481",
    portion: "100",
    calories: { g: "124", vd: "6" },
    carbohydrate: { g: "7,1", vd: "2" },
    sugars: { g: "3,8", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "11", vd: "22" },
    fat: { g: "5,8", vd: "9" },
    saturated: { g: "2", vd: "10" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "1,9", vd: "8" },
    sodium: { g: "271", vd: "14" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "bife-a-parmegiana",
    title: "Bife à Parmegiana",
    category: "Especiais",
    price: "28,99",
    ingredients:
      "Tomate, carne bovina, presunto, ovo, queijo muçarela, óleo de soja, farinha de rosca, cebola, sal, extrato de tomate, farinha de trigo, alho, manjericão, salsa, orégano, tomilho e manjerona.",
    allergic:
      "Alérgicos: Contém derivados de leite, soja e ovo. Contém glúten.",
    weight: "642",
    portion: "100",
    calories: { g: "231", vd: "12" },
    carbohydrate: { g: "7,1", vd: "2" },
    sugars: { g: "1,6", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "16", vd: "32" },
    fat: { g: "15", vd: "23" },
    saturated: { g: "4,4", vd: "22" },
    trans: { g: "0,2", vd: "10" },
    fiber: { g: "1", vd: "5" },
    sodium: { g: "695", vd: "35" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "feijoada",
    title: "Feijoada",
    category: "Especiais",
    price: "26,99",
    ingredients:
      "Dobradinha fresca, feijão branco, tomate, cebola, linguiça, batata, azeitona, óleo de soja, alho, extrato de tomate, sal, salsa, tomilho e manjerona.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "911",
    portion: "100",
    calories: { g: "355", vd: "44" },
    carbohydrate: { g: "40", vd: "33" },
    sugars: { g: "0,4", vd: "" },
    added: { g: "0,3", vd: "1" },
    protein: { g: "17", vd: "84" },
    fat: { g: "16", vd: "62" },
    saturated: { g: "5,7", vd: "70" },
    trans: { g: "0,2", vd: "30" },
    fiber: { g: "14", vd: "144" },
    sodium: { g: "816", vd: "102" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "fricasse",
    title: "Fricassê",
    category: "Especiais",
    price: "27,99",
    ingredients:
      "Filé de frango, creme de leite, molho pomodoro (tomate, cebola, extrato de tomate, óleo de soja, alho, sal, salsa, tomilho e manjerona), milho, requeijão, cebola. Óleo de soja, sal, alho e louro.",
    allergic:
      "Alérgicos: Contém derivados de soja e derivados de leite. Pode conter ovo. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "331", vd: "17" },
    carbohydrate: { g: "6,4", vd: "2" },
    sugars: { g: "3,4", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "25", vd: "50" },
    fat: { g: "23", vd: "35" },
    saturated: { g: "13", vd: "65" },
    trans: { g: "0,7", vd: "35" },
    fiber: { g: "0,7", vd: "35" },
    sodium: { g: "587", vd: "29" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "mocoto",
    title: "Mocotó",
    category: "Especiais",
    price: "26,99",
    ingredients:
      "Dobradinha fresca, feijão branco, tomate, cebola, linguiça, batata, azeitona, óleo de soja, alho, extrato de tomate, sal, salsa, tomilho e manjerona.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "497",
    portion: "100",
    calories: { g: "135", vd: "17" },
    carbohydrate: { g: "6,1", vd: "5" },
    sugars: { g: "1,1", vd: "" },
    added: { g: "0,3", vd: "1" },
    protein: { g: "5,6", vd: "28" },
    fat: { g: "10", vd: "40" },
    saturated: { g: "2,5", vd: "32" },
    trans: { g: "0,3", vd: "45" },
    fiber: { g: "0,7", vd: "7" },
    sodium: { g: "225", vd: "28" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "almondegas-arroz-legumes",
    title: "Almôndegas com Arroz e Legumes",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Almôndegas (tomate, carne bovina, cebola, extrato de tomate, alho, sal, óleo de soja, pimenta em pó, salsa, tomilho e manjerona), legumes sauté (abobrinha, cenoura, couve-flor, brócolis, tomate, azeite de oliva e sal) e arroz (arroz, cebola, sal, alho e louro).",
    allergic:
      "Alérgicos: Contém derivados de soja e trigo. Pode conter leite e ovo. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "141", vd: "35" },
    carbohydrate: { g: "12", vd: "20" },
    sugars: { g: "1", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "8,7", vd: "88" },
    fat: { g: "6,4", vd: "49" },
    saturated: { g: "1,6", vd: "41" },
    trans: { g: "0", vd: "10" },
    fiber: { g: "1,1", vd: "23" },
    sodium: { g: "305", vd: "76" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "bife-milanesa-arroz-feijao",
    title: "Bife à Milanesa com Arroz e Feijão",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Arroz (arroz, cebola, sal, alho e louro), bife à milanesa (carne bovina, ovo, óleo de soja, farinha de rosca, farinha de trigo, sal e pimenta em pó) e feijão (feijão, cebola, óleo de soja, alho, sal, tomilho, manjerona e louro).",
    allergic:
      "Alérgicos: Contém ovo e derivados de trigo e soja. Pode conter leite. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "229", vd: "49" },
    carbohydrate: { g: "26", vd: "37" },
    sugars: { g: "0,2", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "14", vd: "116" },
    fat: { g: "7,8", vd: "52" },
    saturated: { g: "1,9", vd: "41" },
    trans: { g: "0", vd: "25" },
    fiber: { g: "3,1", vd: "52" },
    sodium: { g: "311", vd: "67" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "carne-moida-arroz-feijao-moranga",
    title: "Carne Moída com Arroz, Feijão e Moranga",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Carne moída (carne bovina, molho pomodoro, cebola, cenoura, milho, ervilha, óleo de soja, sal, salsa e tomilho), feijão (feijão, cebola, óleo de soja, alho, sal, tomilho, manjerona e louro), arroz (arroz, cebola, sal, alho e louro) e moranga (moranga e alho).",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten. Contém lactose.",
    weight: "500",
    portion: "100",
    calories: { g: "133", vd: "33" },
    carbohydrate: { g: "20", vd: "33" },
    sugars: { g: "2,4", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "7,1", vd: "72" },
    fat: { g: "3", vd: "23" },
    saturated: { g: "0,8", vd: "20" },
    trans: { g: "0", vd: "10" },
    fiber: { g: "3,8", vd: "76" },
    sodium: { g: "274", vd: "69" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "carne-moida-legumes-farofa",
    title: "Carne Moída com Legumes e Farofa",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Carne moída (carne bovina, molho pomodoro, cebola, cenoura, milho, ervilha, óleo de soja, sal, salsa e tomilho), legumes sauté (abobrinha, cenoura, couve-flor, brócolis, tomate, azeite de oliva e sal) e farofa (farinha de mandioca, cebola, manteiga, alho, óleo de soja e sal).",
    allergic:
      "Alérgicos: Contém derivados de leite, soja e trigo. Pode conter ovo. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "253", vd: "63" },
    carbohydrate: { g: "29", vd: "49" },
    sugars: { g: "2,7", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "5,7", vd: "56" },
    fat: { g: "13", vd: "100" },
    saturated: { g: "4,8", vd: "120" },
    trans: { g: "0,3", vd: "70" },
    fiber: { g: "3,5", vd: "68" },
    sodium: { g: "534", vd: "134" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "frango-xadrez-arroz-legumes-saute",
    title: "Frango Xadrez com Arroz e Legumes Sauté",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Frango xadrez (filé de frango, pimentão verde, cebola, shoyo, amendoim, água, óleo de soja e amido de milho), legumes sauté (abobrinha, cenoura, couve-flor, brócolis, tomate, azeite de oliva e sal) e arroz (arroz, cebola, sal, alho e louro).",
    allergic:
      "Alérgicos: Contém amendoim, derivados de soja e trigo. Pode conter leite e ovo. Contém glúten.",
    weight: "500",
    portion: "100",
    calories: { g: "159", vd: "40" },
    carbohydrate: { g: "15", vd: "24" },
    sugars: { g: "1,7", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "9,1", vd: "92" },
    fat: { g: "7,6", vd: "58" },
    saturated: { g: "1,2", vd: "31" },
    trans: { g: "0", vd: "10" },
    fiber: { g: "2,1", vd: "40" },
    sodium: { g: "566", vd: "142" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "fricasse-arroz-batata-rustica",
    title: "Fricassê com Arroz e Batata Rústica",
    category: "Prática",
    price: "20,00",
    ingredients:
      "Fricassê (filé de frango, creme de leite, molho pomodoro (tomate, cebola, extrato de tomate, óleo de soja, alho, sal, salsa, tomilho e manjerona), milho, requeijão, cebola, óleo de soja, sal, alho e louro), arroz (arroz, cebola, sal, alho e louro) e batata rústica (batata, óleo de soja, sal e alecrim).",
    allergic:
      "Alérgicos: Contém derivados de leite e soja. Pode conter ovo. Contém glúten. Contém lactose.",
    weight: "500",
    portion: "100",
    calories: { g: "223", vd: "56" },
    carbohydrate: { g: "21", vd: "35" },
    sugars: { g: "1,8", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "12", vd: "116" },
    fat: { g: "10", vd: "78" },
    saturated: { g: "5,4", vd: "135" },
    trans: { g: "0,3", vd: "70" },
    fiber: { g: "1,4", vd: "28" },
    sodium: { g: "559", vd: "140" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "panqueca-frango-arroz-legumes",
    title: "Panqueca de Frango com Requeijão, Arroz Integral e Legumes Sauté",
    category: "Prática",
    price: "21,99",
    ingredients:
      "Panqueca de frango com requeijão (tomate, carne de frango, requeijão cremoso, farinha de trigo, cebola, óleo de soja, ovo, extrato de tomate, alho, sal, salsa, tomilho, manjerona e noz moscada), legumes sauté (abobrinha, cenoura, couve-flor, brócolis, tomate, azeite de oliva e sal) e arroz integral (arroz integral, cebola, sal, alho, óleo de soja e louro).",
    allergic:
      "Alérgicos: Contém derivados de soja e leite. Pode conter ovo. Contém glúten. Contém lactose.",
    weight: "500",
    portion: "100",
    calories: { g: "139", vd: "35" },
    carbohydrate: { g: "10", vd: "16" },
    sugars: { g: "1,6", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "5,8", vd: "58" },
    fat: { g: "8,9", vd: "69" },
    saturated: { g: "2,1", vd: "55" },
    trans: { g: "0", vd: "10" },
    fiber: { g: "1,6", vd: "32" },
    sodium: { g: "325", vd: "81" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "lasanha-4-queijos",
    title: "Lasanha 4 Queijos",
    category: "Massas",
    price: "39,99",
    ingredients:
      "Leite, massa de lasanha, queijo muçarela, queijo provolone, queijo colonial, queijo gorgonzola, cebola, farinha de trigo, manteiga sem sal, louro, cravo da índia e noz-moscada.",
    allergic:
      "Alérgicos: Contém derivados de leite e trigo. Pode conter ovo. Contém glúten. Contém lactose.",
    weight: "657",
    portion: "100",
    calories: { g: "246", vd: "12" },
    carbohydrate: { g: "17", vd: "6" },
    sugars: { g: "3,3", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "14", vd: "28" },
    fat: { g: "14", vd: "22" },
    saturated: { g: "8,3", vd: "42" },
    trans: { g: "0,3", vd: "15" },
    fiber: { g: "0", vd: "0" },
    sodium: { g: "476", vd: "24" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "lasanha-bolonhesa",
    title: "Lasanha à Bolonhesa",
    category: "Massas",
    price: "37,99",
    ingredients:
      "Leite, carne bovina, queijo muçarela, tomate, massa de lasanha, cebola, farinha de trigo, manteiga sem sal, óleo de soja, queijo parmesão, extrato de tomate, alho, sal, salsa, louro, tomilho, manjerona, cravo da índia e noz-moscada.",
    allergic:
      "Alérgicos: Contém derivados de soja, leite e trigo. Pode conter ovo. Contém glúten. Contém lactose.",
    weight: "832",
    portion: "100",
    calories: { g: "199", vd: "10" },
    carbohydrate: { g: "11", vd: "4" },
    sugars: { g: "3,8", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "13", vd: "26" },
    fat: { g: "11", vd: "17" },
    saturated: { g: "6,1", vd: "31" },
    trans: { g: "0,3", vd: "15" },
    fiber: { g: "0,6", vd: "2" },
    sodium: { g: "330", vd: "17" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "creme-moranga-gengibre",
    title: "Creme de Moranga com Gengibre",
    category: "Sopas",
    price: "20,00",
    ingredients:
      "Moranga, cebola, alho, óleo de soja, sal, tomilho, manjerona, louro e gengibre.",
    allergic:
      "Alérgicos: Contém derivados de soja. Pode conter leite e ovo. Contém glúten.",
    weight: "825",
    portion: "100",
    calories: { g: "38", vd: "5" },
    carbohydrate: { g: "6,4", vd: "5" },
    sugars: { g: "3,7", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "1", vd: "5" },
    fat: { g: "1,3", vd: "5" },
    saturated: { g: "0,2", vd: "3" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "1,4", vd: "14" },
    sodium: { g: "285", vd: "36" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "sopa-legumes",
    title: "Sopa de Legumes",
    category: "Sopas",
    price: "20,00",
    ingredients:
      "Moranga, carne bovina, batata, cenoura, batata-doce, cebola, couve, alho, pimenta em pó, sal, tomilho e manjerona.",
    allergic: "Alérgicos: Pode conter leite e ovo. Contém glúten.",
    weight: "825",
    portion: "100",
    calories: { g: "74", vd: "9" },
    carbohydrate: { g: "10", vd: "9" },
    sugars: { g: "3,1", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "4,9", vd: "24" },
    fat: { g: "1,4", vd: "6" },
    saturated: { g: "0,7", vd: "9" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "1,8", vd: "18" },
    sodium: { g: "71", vd: "9" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
  {
    slug: "legumes-saute",
    title: "Legumes Sauté",
    category: "Vegana",
    price: "16,99",
    ingredients:
      "Abobrinha, cenoura, couve-flor, brócolis, tomate, azeite de oliva e sal.",
    allergic: "Alérgicos: Pode conter leite e ovo. Contém glúten.",
    weight: "373",
    portion: "100",
    calories: { g: "151", vd: "8" },
    carbohydrate: { g: "6,5", vd: "2" },
    sugars: { g: "2,8", vd: "" },
    added: { g: "0", vd: "0" },
    protein: { g: "2,2", vd: "4" },
    fat: { g: "14", vd: "22" },
    saturated: { g: "2,1", vd: "11" },
    trans: { g: "0", vd: "0" },
    fiber: { g: "2,7", vd: "11" },
    sodium: { g: "443", vd: "22" },
    conservation:
      "O prazo de validade varia segundo a temperatura de conservação. Mantenha congelado à temperatura igual ou inferior a -18ºC (freezer). Depois de descongelado, manter em temperatura de até 5ºC (refrigerador) e consumir em até 3 dias. Descongelar somente no forno ou refrigeração.",
    preparation:
      "Leve o alimento ainda congelado, na embalagem original sem a tampa, ao forno preaquecido à temperatura de 180ºC por aproximadamente 12 minutos e estará pronto para servir.",
  },
];
